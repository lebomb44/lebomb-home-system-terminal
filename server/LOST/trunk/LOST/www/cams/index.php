...
{
  openCams("http://bourdilot.no-ip.org:85", "Facade", (screen.availHeight)/2,(screen.availWidth)/2,(screen.availHeight)/2-50,(screen.availWidth)/2);
}
...
<h1>Photos enregistrees :</h1>
<?php
$dir_nom = '.'; // dossier liste (pour lister le repertoire courant : $dir_nom = '.'  --> ('point')
$dir = opendir($dir_nom) or die('Erreur de listage : le repertoire n\'existe pas'); // on ouvre le contenu du dossier courant
...
if(!empty($dossier)) {
	sort($dossier); // pour le tri croissant, rsort() pour le tri decroissant
...
if(!empty($fichier)){
	sort($fichier);// pour le tri croissant, rsort() pour le tri decroissant
