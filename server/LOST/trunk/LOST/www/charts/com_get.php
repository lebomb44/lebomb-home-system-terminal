<?php
  ...
  echo "<Lost>";
  if(!isset($_GET["tab"]) || !isset($_GET["year"]) || !isset($_GET["month"]) || !isset($_GET["day"]))
  {
    echo "Arguments tab, year, month or day not set";
    echo "</Lost>";
    exit();
  }
  ...
  $requete  = "SELECT * FROM ".$_GET["tab"]." WHERE `DATE` BETWEEN '".$year."-".sprintf('%02d',$month)."-".sprintf('%02d',$day)." 00:00:00' AND '".$year."-".sprintf('%02d',$month)."-".sprintf('%02d',$day)." 23:59:59' ORDER BY DATE LIMIT 0,100";
